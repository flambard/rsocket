...
start_link_channel_requester(StreamID, Request, Handler, N) ->
    Options = [{recv_credits, N}, {recv_state, open}, {send_state, open}],
    Name = {via, gproc, {n, l, {rsocket_stream, self(), StreamID}}},
    gen_server:start_link(Name, ?MODULE, [StreamID, self(), Request, Handler, Options], []).

start_link_channel_responder(StreamID, Request, Handler, N) ->
    Options = [{send_credits, N}, {recv_state, open}, {send_state, open}],
    Name = {via, gproc, {n, l, {rsocket_stream, self(), StreamID}}},
    gen_server:start_link(Name, ?MODULE, [StreamID, self(), Request, Handler, Options], []).

start_link_stream_requester(StreamID, Request, Handler, N) ->
    Options = [{recv_credits, N}, {recv_state, open}, {send_state, completed}],
    Name = {via, gproc, {n, l, {rsocket_stream, self(), StreamID}}},
    gen_server:start_link(Name, ?MODULE, [StreamID, self(), Request, Handler, Options], []).

start_link_stream_responder(StreamID, Request, Handler, N) ->
    Options = [{recv_state, completed}, {send_state, open}],
    Name = {via, gproc, {n, l, {rsocket_stream, self(), StreamID}}},
    {ok, Pid} =
        gen_server:start_link(Name, ?MODULE, [StreamID, self(), Request, Handler, Options], []),
...
handle_call({send_payload, Payload, Options}, _From, State) ->
    ...
    case proplists:is_defined(complete, Options) of
        ...
        true ->
            ...
            case RecvState of
                completed ->
                    {stop, completed, ok, CompletedState};
                open ->
                    {reply, ok, CompletedState}
            end
    end.
